...
```python
# 导入枚举类型
from qe.lib import Algorithm, Exchange, MarketType, OrderSide, StrategyType, MarginType, Category, MasterOrderStatus
...
print("保证金类型:", [margin.value for margin in MarginType])   # ['U', 'C']
print("交易品种:", [category.value for category in Category])   # ['spot', 'perp', 'perp_cm']
print("母单状态:", [status.value for status in MasterOrderStatus])  # ['NEW', 'COMPLETED']
...
所有接口均需 HMAC-SHA256 签名鉴权，公共必传参数为 `binding_id`（交易所 API Key 绑定 UUID，通过 `list_exchange_apis()` 获取）。

**限频规则：** 同一 `binding_id` 10 分钟内最多 60 次请求。
...
| 参数名 | 类型 | 是否必传 | 描述 |
|--------|------|----------|------|
| binding_id | str | 是 | 交易所 API Key 绑定 UUID |

**响应字段：**

| 字段名 | 类型 | 描述 |
|--------|------|------|
| balances | list | 现货余额列表 |
| ├─ asset | str | 资产符号（如 BTC、USDT） |
| ├─ free | str | 可用余额 |
| ├─ locked | str | 冻结余额 |
| exchange | str | 交易所名称 |
| accountType | str | 账户类型（SPOT） |
| updateTime | str | 余额更新时间 |

**示例代码：**

```python
result = client.get_account_balance("your-binding-id")
for b in result["balances"]:
    print(f"资产: {b['asset']}, 可用: {b['free']}, 冻结: {b['locked']}")
```
...
| 字段名 | 类型 | 描述 |
|--------|------|------|
| balances | list | 合约余额列表 |
| ├─ asset | str | 资产符号 |
| ├─ walletBalance | str | 钱包余额 |
| ├─ availableBalance | str | 可用余额 |
| ├─ crossWalletBalance | str | 全仓钱包余额 |
| ├─ crossUnPnl | str | 全仓未实现盈亏 |
| ├─ marginBalance | str | 保证金余额 |
| ├─ maxWithdrawAmount | str | 最大可转出余额 |
| exchange | str | 交易所名称 |
| accountType | str | 账户类型（FUTURES） |
| updateTime | str | 余额更新时间 |

**示例代码：**

```python
result = client.get_margin_balance("your-binding-id")
for b in result["balances"]:
    print(f"资产: {b['asset']}, 可用: {b['availableBalance']}")
```
...
| balances | list | 余额列表 |
| ├─ asset | str | 资产符号 |
| ├─ totalWalletBalance | str | 总钱包余额 |
| ├─ crossMarginBorrowed | str | 全仓杠杆借入 |
| ├─ crossMarginFree | str | 全仓杠杆可用 |
| ├─ crossMarginInterest | str | 全仓杠杆利息 |
| ├─ crossMarginLocked | str | 全仓杠杆锁定 |
| ├─ umWalletBalance | str | U本位合约钱包余额 |
| ├─ umUnrealizedPnl | str | U本位合约未实现盈亏 |
| ├─ cmWalletBalance | str | 币本位合约钱包余额 |
| ├─ cmUnrealizedPnl | str | 币本位合约未实现盈亏 |
| ├─ updateTime | int | 更新时间（毫秒时间戳） |
| ├─ negativeBalance | str | 负余额 |

**示例代码：**

```python
result = client.get_pv1_balance("your-binding-id")
for b in result["balances"]:
    print(f"资产: {b['asset']}, 总余额: {b['totalWalletBalance']}")
```
...
| ├─ mmr | str | 维持保证金 |
| ├─ mgnRatio | str | 保证金率 |
| ├─ notionalUsd | str | 名义美元价值 |
| ├─ ordFroz | str | 订单冻结 |
| ├─ upl | str | 未实现盈亏 |
| ├─ uTime | str | 更新时间 |
| ├─ details | list | 各币种余额详情 |
| │ ├─ ccy | str | 币种 |
| │ ├─ eq | str | 权益 |
| │ ├─ eqUsd | str | 权益（USD） |
| │ ├─ availBal | str | 可用余额 |
...
```python
result = client.get_okx_account_balance("your-binding-id")
for d in result["data"]:
    print(f"总权益: {d['totalEq']}, 可用权益: {d['availEq']}")
```
...
```python
result = client.get_fapi_position_side_dial("your-binding-id")
print(f"双向持仓: {result['dualSidePosition']}")
```
...
```python
result = client.get_papi_um_position_side_dual("your-binding-id")
print(f"双向持仓: {result['dualSidePosition']}")
```
...
|--------|------|------|
| data | list | 持仓信息列表 |
| ├─ instId | str | 产品 ID（如 BTC-USDT-SWAP） |
...
```python
result = client.get_okx_account_positions("your-binding-id")
for p in result["data"]:
    print(f"产品: {p['instId']}, 持仓量: {p['pos']}, 未实现盈亏: {p['upl']}")
```
...
|--------|------|----------|------|
| binding_id | str | 是 | 交易所 API Key 绑定 UUID |
| inst_id | str | 是 | 产品 ID（如 BTC-USDT-SWAP） |
| td_mode | str | 是 | 交易模式：cross（全仓）、isolated（逐仓）、cash（现货） |
...
```python
result = client.get_okx_account_max_size(
    "your-binding-id",
    inst_id="BTC-USDT-SWAP",
    td_mode="cross"
)
for d in result["data"]:
    print(f"最大买入: {d['maxBuy']}, 最大卖出: {d['maxSell']}")
```
...
|--------|------|------|
| data | list | 持仓信息列表 |
| ├─ positionId | str | 持仓 ID |
| ├─ portfolioId | str | 投资组合 ID |
| ├─ sym | str | 交易对 |
| ├─ positionSide | str | 持仓方向 |
| ├─ positionQty | str | 持仓数量 |
| ├─ positionValue | str | 持仓价值 |
| ├─ positionMargin | str | 持仓保证金 |
| ├─ positionMm | str | 持仓维持保证金 |
| ├─ unrealizedPnl | str | 未实现盈亏 |
| ├─ unrealizedPnlRate | str | 未实现盈亏率 |
| ├─ avgPrice | str | 平均开仓价 |
| ├─ markPrice | str | 标记价格 |
| ├─ liqPrice | str | 强平价格 |
| ├─ leverage | str | 杠杆倍数 |
| ├─ maxLeverage | str | 最大杠杆 |
| ├─ riskLevel | str | 风险等级 |
| ├─ fee | str | 手续费 |
| ├─ fundingFee | str | 资金费 |
| ├─ tpslOrder | str | 止盈止损订单（JSON 字符串） |
| ├─ createAt | str | 创建时间 |
| ├─ updateAt | str | 更新时间 |
| exchange | str | 交易所名称 |
...
```python
# 查询所有持仓
result = client.get_ltp_position("your-binding-id")

# 查询指定交易对持仓
result = client.get_ltp_position("your-binding-id", sym="BTCUSDT")

for p in result["data"]:
    print(f"交易对: {p['sym']}, 持仓量: {p['positionQty']}")
```
...
| ├─ direction | str | 持仓方向（buy/sell） |
| ├─ size | float | 持仓数量 |
| ├─ averagePrice | float | 平均开仓价 |
| ├─ markPrice | float | 标记价格 |
| ├─ indexPrice | float | 指数价格 |
| ├─ floatingProfitLoss | float | 浮动盈亏 |
| ├─ totalProfitLoss | float | 总盈亏 |
| ├─ initialMargin | float | 初始保证金 |
| ├─ maintenanceMargin | float | 维持保证金 |
| ├─ estimatedLiquidationPrice | float | 预估强平价格 |
| ├─ leverage | int | 杠杆倍数 |
| ├─ kind | str | 合约类型（future/option） |
| ├─ sizeCurrency | float | 币本位持仓数量 |
| ├─ delta | float | Delta |
| ├─ realizedFunding | float | 已实现资金费 |
| ├─ realizedProfitLoss | float | 已实现盈亏 |
...
```python
result = client.get_deribit_position("your-binding-id")
for p in result["data"]:
    print(f"合约: {p['instrumentName']}, 方向: {p['direction']}, 数量: {p['size']}")
```
...
| 字段名 | 类型 | 描述 |
|--------|------|------|
| tradeGroupId | int | 交易组 ID |
| assets | list | 资产列表 |
| ├─ asset | str | 资产符号 |
| ├─ crossWalletBalance | str | 全仓钱包余额 |
| ├─ crossUnPnl | str | 全仓未实现盈亏 |
| ├─ maintMargin | str | 维持保证金 |
| ├─ initialMargin | str | 初始保证金 |
| ├─ positionInitialMargin | str | 持仓初始保证金 |
| ├─ openOrderInitialMargin | str | 挂单初始保证金 |
| ├─ updateTime | int | 更新时间（毫秒时间戳） |
| positions | list | 持仓列表 |
| ├─ symbol | str | 交易对 |
...
```python
result = client.get_um_account("your-binding-id")
print(f"交易组ID: {result['tradeGroupId']}, 持仓数: {len(result['positions'])}")
```
...
```python
result = client.get_cm_account("your-binding-id")
print(f"资产数: {len(result['assets'])}, 持仓数: {len(result['positions'])}")
```
...
| accountType | str | 账户类型 |
| uniMmr | str | 统一维持保证金率 |
| accountEquity | str | 账户权益 |
| actualEquity | str | 实际权益 |
| accountInitialMargin | str | 账户初始保证金 |
| accountMaintMargin | str | 账户维持保证金 |
| accountStatus | str | 账户状态 |
| virtualMaxWithdrawAmount | str | 虚拟最大可提取金额 |
| totalAvailableBalance | str | 总可用余额 |
| totalMarginOpenLoss | str | 总保证金未平仓亏损 |
| updateTime | str | 更新时间 |

**示例代码：**

```python
result = client.get_pv1_account("your-binding-id")
print(f"账户权益: {result['accountEquity']}, 统一维持保证金率: {result['uniMmr']}")
```
...
```python
result = client.get_dapi_account("your-binding-id")
print(f"资产数: {len(result['assets'])}, 持仓数: {len(result['positions'])}")
```
...
|--------|------|------|
| totalWalletBalance | str | 总钱包余额 |
| totalUnrealizedProfit | str | 总未实现盈亏 |
| totalMarginBalance | str | 总保证金余额 |
| availableBalance | str | 可用余额 |
| maxWithdrawAmount | str | 最大可提取金额 |
| totalInitialMargin | str | 总初始保证金 |
| totalMaintMargin | str | 总维持保证金 |
| totalCrossWalletBalance | str | 总全仓钱包余额 |
| totalCrossUnPnl | str | 总全仓未实现盈亏 |
| assets | list | 资产列表 |
| ├─ asset | str | 资产符号 |
| ├─ walletBalance | str | 钱包余额 |
| ├─ unrealizedProfit | str | 未实现盈亏 |
...
```python
result = client.get_fapi_account("your-binding-id")
print(f"总钱包余额: {result['totalWalletBalance']}, 可用余额: {result['availableBalance']}")
```
...
| marginLevel | str | 保证金水平 |
| totalAssetOfBtc | str | 总资产（BTC 计） |
| totalLiabilityOfBtc | str | 总负债（BTC 计） |
| totalNetAssetOfBtc | str | 总净资产（BTC 计） |
| userAssets | list | 各币种资产详情 |
| ├─ asset | str | 资产符号 |
| ├─ free | str | 可用余额 |
| ├─ locked | str | 锁定余额 |
| ├─ borrowed | str | 借入金额 |
...
```python
result = client.get_cross_margin_account_detail("your-binding-id")
print(f"保证金水平: {result['marginLevel']}, 总净资产(BTC): {result['totalNetAssetOfBtc']}")
```
...
| data | list | 账户信息列表 |
| ├─ portfolioId | str | 投资组合 ID |
| ├─ exchangeType | str | 交易所类型 |
| ├─ equity | str | 权益 |
| ├─ maintainMargin | str | 维持保证金 |
| ├─ positionValue | str | 持仓价值 |
| ├─ uniMmr | str | 统一维持保证金率 |
| ├─ riskRatio | str | 风险率 |
| ├─ accountStatus | str | 账户状态 |
| ├─ availableMargin | str | 可用保证金 |
| ├─ validMargin | str | 有效保证金 |
| ├─ frozenMargin | str | 冻结保证金 |
| ├─ upnl | str | 未实现盈亏 |
| ├─ positionMode | str | 持仓模式 |
| exchange | str | 交易所名称 |

**示例代码：**

```python
result = client.get_ltp_account("your-binding-id")
for a in result["data"]:
    print(f"组合ID: {a['portfolioId']}, 权益: {a['equity']}, 风险率: {a['riskRatio']}")
```
...
| ├─ borrow | str | 借币 |
| ├─ debt | str | 负债 |
| ├─ marginValue | str | 保证金价值 |
| ├─ indexPrice | str | 指数价格 |
| ├─ maxTransferable | str | 最大可转出金额 |
| ├─ upnl | str | 未实现盈亏 |
| ├─ equityValue | str | 权益价值 |
| ├─ createAt | str | 创建时间 |
| ├─ updateAt | str | 更新时间 |
| exchange | str | 交易所名称 |
...
```python
result = client.get_ltp_portfolio_asset("your-binding-id")
for a in result["data"]:
    print(f"币种: {a['coin']}, 可用: {a['available']}, 权益: {a['equity']}")
```
...
| 字段名 | 类型 | 描述 |
|--------|------|------|
| data | list | 账户信息列表（按币种分组） |
| ├─ currency | str | 币种（如 BTC、ETH） |
| ├─ equity | float | 权益 |
| ├─ balance | float | 余额 |
| ├─ availableFunds | float | 可用资金 |
| ├─ availableWithdrawalFunds | float | 可用提现资金 |
| ├─ marginBalance | float | 保证金余额 |
| ├─ initialMargin | float | 初始保证金 |
| ├─ maintenanceMargin | float | 维持保证金 |
| ├─ lockedBalance | float | 锁定余额 |
| ├─ totalPl | float | 总盈亏 |
| ├─ sessionUpl | float | 会话未实现盈亏 |
| ├─ sessionRpl | float | 会话已实现盈亏 |
| ├─ futuresPl | float | 期货盈亏 |
| ├─ optionsValue | float | 期权价值 |
| ├─ optionsDelta | float | 期权 Delta |
| ├─ optionsGamma | float | 期权 Gamma |
| ├─ optionsVega | float | 期权 Vega |
| ├─ optionsTheta | float | 期权 Theta |
| ├─ deltaTotal | float | 总 Delta |
| ├─ marginModel | str | 保证金模型 |
...
```python
result = client.get_deribit_account("your-binding-id")
for a in result["data"]:
    print(f"币种: {a['currency']}, 权益: {a['equity']}, 可用资金: {a['availableFunds']}")
```
...
| orderNotional | float | 否*   | 按价值下单时的金额，以计价币种为单位（如ETHUSDT为USDT数量），与 totalQuantity 二选一，输入范围：>0。Deribit 下单 BTCUSD/ETHUSD 与 Binance 下单 `perp_cm` 时当前字段不可用 |
| **下单模式参数** |
...
orders = client.get_master_orders(
    page=1,
    pageSize=20,
    status=MasterOrderStatus.NEW,  # 执行中的订单
    symbol="BTCUSDT",
...
|--------|------|----------|------|
| symbol | str | 否 | 交易对筛选 |
| category | str | 否 | 交易品种（spot、perp 或 perp_cm） |
...
| Strategy | str | 算法类型 |
| Symbol | str | 交易对 |
| Category | str | 交易类型（spot、perp 或 perp_cm） |
| Side | str | 交易方向 |
| Date | str | 母单创建日期 |
...
def cancel_all_active_orders(client):
    """取消所有活跃订单"""
    orders = client.get_master_orders(status=MasterOrderStatus.NEW)
    cancelled_count = 0
